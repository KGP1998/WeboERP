...
namespace DairyWeb.Repository
{
    public class TransactionRepository : BaseRepository<TransactionMaster>, ITransactionRepository
    {
        ...
        public Tuple<bool, string> UpsertTransaction(TransactionMaster transaction)
        {
            ...
            p.Add("@PaymentType", transaction.PaymentType);
        ...
        public int GetOutStandingAmountForCustomer(int customerId)
        {
            ...
            using (var con = CommonOperations.GetConnection())
            {
                var p = new DynamicParameters();
                p.Add("@CustomerId", customerId);
                outstandingAmount = con.Query<int>("GetOutStandingAmountForCustomer", p, commandType: System.Data.CommandType.StoredProcedure).FirstOrDefault();
            }
