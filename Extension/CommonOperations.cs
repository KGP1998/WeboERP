...
namespace DairyWeb.Extension
{
    public class CommonOperations
    {
        public static SqlConnection GetConnection()
        {
            ...
            //}
            var con = System.Configuration.ConfigurationManager.ConnectionStrings["DairyContext"].ConnectionString;
        ...
        public static string SendSms(string MessageText, string ContactNos, string User = "Nitin44", string Password = "e1bb8d7f42XX", string SenderId = "APXCOM")
        {
            //Your message to send, Add URL encoding here.
            string SMS = HttpUtility.UrlEncode(MessageText);

            //Prepare you post parameters
            StringBuilder sbPostData = new StringBuilder();
            sbPostData.AppendFormat("user={0}", User);
            sbPostData.AppendFormat("&password={0}", Password);
            sbPostData.AppendFormat("&mobiles={0}", ContactNos);
            sbPostData.AppendFormat("&sms={0}", SMS);
            sbPostData.AppendFormat("&senderid={0}", SenderId);
            sbPostData.AppendFormat("&accusage={0}", "1");

            //Call Send SMS API
            string sendSMSUrl = "http://apex.mysmsapps.co.in/sendsms.jsp?";
